...
/**
 *  PHP wrapper for the OAuth 2.0 protocol.
 *
    ...
    */
    ...
    public function __construct($client_id, $client_secret, $client_auth = self::AUTH_TYPE_URI, $certificate_file = null)
    {
        if (!extension_loaded('curl')) {
            throw new Exception('PHP extension curl must be installed.', Exception::CURL_NOT_FOUND);
        }
        ...
        if (!empty($this->certificate_file)  && !is_file($this->certificate_file)) {
            throw new InvalidArgumentException('The certificate file not found', InvalidArgumentException::CERTIFICATE_NOT_FOUND);
        }
    }
     ...
     * @param array  $extra_parameters  Array of extra parameters
